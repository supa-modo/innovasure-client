/**
 * Agent Details Modal
 * Comprehensive agent management with tabs
 */
...
const AgentModal = ({ isOpen, onClose, agent, onUpdate }: AgentModalProps) => {
  ...
  const handleKYCAction = async (
    ...
    setNotification({
      ...
      message:
        action === "approved"
          ? "Are you sure you want to approve this agent's KYC?"
          : action === "rejected"
            ? "Please provide a reason for rejection."
            : "Please provide a reason for flagging.",
      onConfirm: async () => {
        try {
          await updateAgentKYC(agent.id, action);
          setNotification({
            isOpen: true,
            type: "success",
            title: "KYC Updated",
            message: `Agent KYC has been ${action} successfully.`,
            onConfirm: undefined,
          });
          onUpdate?.();
        } catch (err: any) {
  ...
  const handleToggleStatus = async () => {
    ...
    setNotification({
      ...
      message: `Are you sure you want to ${agent.user?.status === "active" ? "deactivate" : "activate"} this agent?`,
      onConfirm: async () => {
        try {
          await toggleAgentStatus(
            agent.id,
            agent.user?.status === "active" ? "inactive" : "active"
          );
          setNotification({
            isOpen: true,
            type: "success",
            title: "Status Updated",
            message: `Agent has been ${agent.user?.status === "active" ? "deactivated" : "activated"} successfully.`,
            onConfirm: undefined,
          });
          onUpdate?.();
        } catch (err: any) {
  ...
  const handleResetPassword = async () => {
    ...
    setNotification({
      ...
      message:
        "Are you sure you want to reset this agent's password? A temporary password will be generated.",
      onConfirm: async () => {
        try {
          ...
          setNotification({
            ...
            message:
              "Agent password has been reset successfully. The new password has been sent to their email.",
            onConfirm: undefined,
          });
        } catch (err: any) {
          setNotification({
                  ...
                  <div className="space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-gray-900">
                          {agent.super_agent?.user?.profile?.full_name || "N/A"}
                        </p>
                      </div>
                    </div>
                  ...
                  <div className="space-y-4">
                    <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl p-6 border border-green-100">
                      ...
                      <p className="text-3xl font-bold text-gray-900">
                        KSh {(agent.commissionBalance || 0).toLocaleString()}
                      </p>
                    ...
                    <p className="text-gray-500 text-center py-8">
                      Commission history coming soon
                    </p>
                  </div>
                )}
                      ...
                      <button
                        onClick={() => {
                          setNotification({
                            ...
                            message:
                              "Are you sure you want to permanently deactivate this agent? This action cannot be undone.",
                            onConfirm: undefined,
                          });
                        }}
                        className="px-4 py-2 text-sm font-medium text-white bg-red-600 rounded-lg hover:bg-red-700 transition-colors flex items-center gap-2"
